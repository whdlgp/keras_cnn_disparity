...
def disparity_cnn_mixed_model(input_shape):
    shape=(None, input_shape[1], input_shape[2], input_shape[3])
    left = Input(batch_shape=shape)
    right = Input(batch_shape=shape)

    left_1_mix = concatenate([left, right])

    left_1 = Conv2D(filters=32, kernel_size=3, padding='same')(left_1_mix)
    left_1_pool = MaxPooling2D(2)(left_1)
    left_1_activate = Activation('relu')(left_1_pool)

    left_1_batch = BatchNormalization()(left_1_activate)

    left_2 = Conv2D(filters=62, kernel_size=3, padding='same')(left_1_batch)
    left_2_pool = MaxPooling2D(2)(left_2)
    left_2_activate = Activation('relu')(left_2_pool)

    left_2_batch = BatchNormalization()(left_2_activate)

    left_3 = Conv2D(filters=92, kernel_size=3, padding='same')(left_2_batch)
    left_3_activate = Activation('relu')(left_3)

    left_3_batch = BatchNormalization()(left_3_activate)

    right_1_mix = concatenate([right, left])

    right_1 = Conv2D(filters=32, kernel_size=3, padding='same')(right_1_mix)
    right_1_pool = MaxPooling2D(2)(right_1)
    right_1_activate = Activation('relu')(right_1_pool)

    right_1_batch = BatchNormalization()(right_1_activate)

    right_2 = Conv2D(filters=62, kernel_size=3, padding='same')(right_1_batch)
    right_2_pool = MaxPooling2D(2)(right_2)
    right_2_activate = Activation('relu')(right_2_pool)

    right_2_batch = BatchNormalization()(right_2_activate)

    right_3 = Conv2D(filters=92, kernel_size=3, padding='same')(right_2_batch)
    right_3_activate = Activation('relu')(right_3)

    right_3_batch = BatchNormalization()(right_3_activate)

    merge = concatenate([left_3_batch, right_3_batch])
    ...
    merge_1_batch = BatchNormalization()(merge_1_activate)

    merge_2 = Conv2D(filters=22, kernel_size=3, padding='same')(merge_1_batch)
    merge_2_up = UpSampling2D(2)(merge_2)
    merge_2_activate = Activation('relu')(merge_2_up)

    merge_2_batch = BatchNormalization()(merge_2_activate)

    merge_3 = Conv2D(filters=1, kernel_size=2, padding='same')(merge_2_batch)
    merge_3_activate = Activation('relu')(merge_3)

    merge_3_batch = BatchNormalization()(merge_3_activate)

    model = Model([left, right], merge_3_batch)
    plot_model(model, to_file='model_mixed.png', show_shapes=True, show_layer_names=True)
...
# build model
model = disparity_cnn_mixed_model(test_left.shape)
